...
public interface WellnessMetricRepository extends JpaRepository<WellnessMetric, Long> {
    ...
    // @Query("SELECT wm.dailySteps FROM WellnessMetric wm WHERE wm.employeeId =
    // :employeeId ORDER BY wm.date DESC LIMIT 1")
    // int getDailyStepsByEmployeeId(@Param("employeeId") Long employeeId);

    // @Query("SELECT wm.waterIntake FROM WellnessMetric wm WHERE wm.employeeId =
    // :employeeId ORDER BY wm.date DESC LIMIT 1")
    ...
    // @Query("SELECT wm.mood FROM WellnessMetric wm WHERE wm.employeeId =
    // :employeeId ORDER BY wm.date DESC LIMIT 1")
    // String findLatestMoodByEmployeeId(@Param("employeeId") Long employeeId);

    // @Query("SELECT wm.sleepHours FROM WellnessMetric wm WHERE wm.employeeId =
    // :employeeId ORDER BY wm.date DESC LIMIT 1")
